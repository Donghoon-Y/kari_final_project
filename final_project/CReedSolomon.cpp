...
{	
	rsp.numErrorsPerInterleave	= NULL;
	rsp.s						= NULL;
	rsp.sigma					= NULL;
	rsp.errorMagnitudes			= NULL;
	rsp.errorLocations			= NULL; 
	rsp.D						= NULL;
	rsp.tmpSigma				= NULL;
	rsp.Z						= NULL;
	rsp.log_ptr					= NULL; 
	rsp.antilog_ptr				= NULL;
}
...
//_procRSDecoder.init( 8, 16, 112, 11, 0, 4, 4, 1 );

void
CReedSolomon::init(int BitsPerSymbol, int CorrectableErrors, int mo, int poa, int VirtualFill, 
...
void CReedSolomon::Report( unsigned long& numOfCorrErr, unsigned long& numOfUncorrErr, unsigned long& numOfErrSymbol, float& byteErrRatioEst, unsigned int& numOfFillFrame )
{
	numOfCorrErr	= rsp.numErrorsPerFrame;
	numOfUncorrErr	= rsp.uncorErrsPerFrame;
	numOfErrSymbol	= numOfCorrErr + numOfUncorrErr;
	byteErrRatioEst = 0.0f;
	numOfFillFrame	= rsp.vf;
}
